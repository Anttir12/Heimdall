/*

Copyright (c) Microsoft Corporation. All rights reserved.

Module Name:

    WinSDKVer.h

Abstract:

    Master include file for versioning content that ships in the Windows SDK.

*/

#ifndef _INC_WINSDKVER
#define _INC_WINSDKVER

#pragma once

// This list contains the highest version constants supported by content in the Windows SDK.

#define _WIN32_MAXVER           0x0600
#define _WIN32_WINDOWS_MAXVER   0x0500
#define NTDDI_MAXVER            0x06000100
#define _WIN32_IE_MAXVER        0x0700
#define _WIN32_WINNT_MAXVER     0x0600
#define WINVER_MAXVER           0x0600

#endif

